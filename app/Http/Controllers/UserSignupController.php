...
class UserSignupController extends Controller
{
    ...
    {
        if (Account::count() > 0 && !Utils::ismaxticket()) {
            return redirect()->route('login')->send();
        }

        $this->auth = $auth;

        $captchaConfig = config('maxticket.captcha');
    ...
    {
        $is_maxticket = Utils::ismaxticket();
        return view('Public.LoginAndRegister.Signup', compact('is_maxticket'));
    }
    ...
    {
        $is_maxticket = Utils::ismaxticketCloud();
        $this->validate($request, [
            'email'        => 'required|email|unique:users',
            'password'     => 'required|min:8|confirmed',
            'first_name'   => 'required',
            'last_name'   => 'required',
            'terms_agreed' => $is_maxticket ? 'required' : ''
        ]);
        ...
        $account_data['currency_id'] = config('maxticket.default_currency');
        $account_data['timezone_id'] = config('maxticket.default_timezone');
            ...
            Mail::send(Lang::locale().'.Emails.ConfirmEmail',
                ...
                function ($message) use ($request) {
                    $message->to($request->get('email'), $request->get('first_name'))
                        ->subject(trans("Email.maxticket_register"));
                });
        }
