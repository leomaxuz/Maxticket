...
class EventCheckoutController extends Controller
{
    ...
    public function postValidateTickets(Request $request, $event_id)
    {
         ...
         */
        $order_expires_time = Carbon::now()->addMinutes(config('maxticket.checkout_timeout_after'));
    ...
    public function postCreateOrder(Request $request, $event_id)
    {
        ...
        try {
            ...
            $payment_gateway_config = $ticket_order['account_payment_gateway']->config + [
                                                    'testMode' => config('maxticket.enable_test_payments')];
    ...
    public function completeOrder($event_id, $return_json = true)
    {
        ...
        try {
            ...
            $order->order_status_id = isset($request_data['pay_offline']) ? config('maxticket.order.awaiting_payment') : config('maxticket.order.complete');
