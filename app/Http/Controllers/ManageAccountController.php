...
class ManageAccountController extends MyBaseController
{
    ...
    public function getVersionInfo()
    {
        ...
        try {
            ...
            $response = $http_client->get('https://raw.githubusercontent.com/maxticket/maxticket/master/VERSION');
            $latestVersion = Utils::parse_version((string)$response->getBody());
            $installedVersion = file_get_contents(base_path('VERSION'));
        } catch (\Exception $exception) {
            \Log::warn("Error retrieving the latest maxticket version. ManageAccountController.getVersionInf() try/catch");
            \Log::warn($exception);
            return false;
        }
