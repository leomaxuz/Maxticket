...
use App\maxticket\PaymentUtils;
use Carbon\Carbon;
...
class Ticket extends MyBaseModel
{
    ...
    {
        $format = config('maxticket.default_datetime_format');
        return [
            'title'              => 'required',
    ...
    public function setStartSaleDateAttribute($date)
    {
        if (!$date) {
            ...
            $this->attributes['start_sale_date'] = Carbon::createFromFormat(
                config('maxticket.default_datetime_format'),
                $date
            );
        }
    }
    ...
    public function getBookingFeeAttribute()
    {
        return PaymentUtils::isFree($this->price) ? 0 : round(
            ($this->price * (config('maxticket.ticket_booking_fee_percentage') / 100)) + (config('maxticket.ticket_booking_fee_fixed')),
            2
        );
    }
    ...
    public function getSaleStatusAttribute()
    {
        if ($this->start_sale_date !== null && $this->start_sale_date->isFuture()) {
            return config('maxticket.ticket_status_before_sale_date');
        }

        if ($this->end_sale_date !== null && $this->end_sale_date->isPast()) {
            return config('maxticket.ticket_status_after_sale_date');
        }

        if ((int)$this->quantity_available > 0 && (int)$this->quantity_remaining <= 0) {
            return config('maxticket.ticket_status_sold_out');
        }

        if ($this->event->start_date->lte(Carbon::now())) {
            return config('maxticket.ticket_status_off_sale');
        }

        return config('maxticket.ticket_status_on_sale');
    }
