...
class Organiser extends MyBaseModel implements AuthenticatableContract
{
    ...
    {
        if ($this->logo_path && (file_exists(config('maxticket.cdn_url_user_assets') . '/' . $this->logo_path) || file_exists(public_path($this->logo_path)))) {
            return config('maxticket.cdn_url_user_assets') . '/' . $this->logo_path;
        }

        return config('maxticket.fallback_organiser_logo_url');
    }
    ...
    public function setLogo(UploadedFile $file)
    {
        ...
        // Image Directory
        $imageDirectory = public_path() . '/' . config('maxticket.organiser_images_path');

        // Paths
        $relativePath = config('maxticket.organiser_images_path').'/'.$filename;
