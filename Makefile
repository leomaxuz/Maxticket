...
build:
	docker build --tag maxticket_base --target base .
	docker build --tag maxticket_worker --target worker --cache-from maxticket_base:latest .
	docker build --tag maxticket_web --target web --cache-from maxticket_worker:latest .

build-apache:
	docker build --tag maxticket_base --target base --file Dockerfile-apache .
	docker build --tag maxticket_worker --target worker --cache-from maxticket_base:latest --file Dockerfile-apache .
	docker build --tag maxticket_web --target web --cache-from maxticket_worker:latest --file Dockerfile-apache .
