...
<html lang="en">
    <head>
        ...
        -->
        <title>{{{$event->title}}} - xbayt.uz</title>
        ...
        <meta property="og:image" content="{{config('maxticket.cdn_url_user_assets').'/'.$event->images->first()['image_path']}}" />
        @endif
        <meta property="og:description" content="{{{Str::words(md_to_str($event->description), 20)}}}" />
        <meta property="og:site_name" content="xbayt.uz" />
        <!--[if lt IE 9]>
       ...
       {!!Html::style(config('maxticket.cdn_url_static_assets').'/assets/stylesheet/frontend.css')!!}
        ...
        @if (($event->bg_type == 'image' || $event->bg_type == 'custom_image' || Request::input('bg_img_preview')) && !Request::input('bg_color_preview'))
            <style>
                body {
                    background: url({{(Request::input('bg_img_preview') ? URL::to(Request::input('bg_img_preview')) :  asset(config('maxticket.cdn_url_static_assets').'/'.$event->bg_image_path))}}) no-repeat center center fixed;
    ...
    <body class="maxticket">
        ...
        {!!Html::script(config('maxticket.cdn_url_static_assets').'/assets/javascript/frontend.js')!!}
